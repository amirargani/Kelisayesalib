...
{
    [AllowAnonymous]
    public class NewDetailsInfoViewModel : PageModel
    {
        private readonly APPClinetDbContext _db;

        public NewDetailsInfoViewModel(APPClinetDbContext db)
        {
            _db = db;
        }
        ...
        public IList<NewImages> NewImages { get; set; }

        public async Task<IActionResult> OnGetAsync(int id, string title)
        {
            var detail = await _db.Tbl_NewDetails.Where(a => a.Id == id && a.Title == title.Replace("§", " ").ToString() && a.ActivePassive == true).FirstOrDefaultAsync();
            if (detail == null)
            {
                return Redirect(Url.Content("~/" + "Error"));
            }
            NewImages = await _db.Tbl_NewImages.Where(i => i.DetailId.Equals(detail.Id) && i.ActivePassive == true).ToListAsync();
            var subcategories = await _db.Tbl_NewSubCategories.FirstOrDefaultAsync(c => c.Id.Equals(detail.SubCategoryId));
            var categories = await _db.Tbl_NewCategories.FirstOrDefaultAsync(c => c.Id.Equals(detail.CategoryId));
            ...
            _db.Tbl_NewDetails.Update(detail);
        ...
        {
            var image = _db.Tbl_NewImages.Where(a => a.Id == id).FirstOrDefault();
