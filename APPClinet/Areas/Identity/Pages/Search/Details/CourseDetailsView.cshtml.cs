...
namespace APPClinet.Areas.Identity.Pages.Search.Details
{
    ...
    public class CourseDetailsViewModel : PageModel
    {
        ...
        {
            var subcategories = await _db.Tbl_CourseSubCategories.FirstOrDefaultAsync(c => c.TitleSubCategory.Equals(titlesubcategory));
            ...
            details = await _db.Tbl_CourseDetails.Where(a => a.SubCategoryId == subcategories.Id && a.CategoryId == categories.Id && a.ActivePassive == true).ToListAsync();
            if (details.Count == 0)
            {
                return Redirect(Url.Content("~/" + "Error"));
            }
            // ASP.NET Core Razor Pages - Pagination
            pages = _iuser.OnGetSearchCoursePagesDetails(titlesubcategory, pageid);
            ...
            _db.Tbl_CourseSubCategories.Update(subcategories);
