...
{
    [AllowAnonymous]
    public class CourseDetailsInfoViewModel : PageModel
    {
        private readonly APPClinetDbContext _db;

        public CourseDetailsInfoViewModel(APPClinetDbContext db)
        {
            _db = db;
        }
        ...
        public IList<CourseVideos> courseVideos { get; set; }

        public async Task<IActionResult> OnGetAsync(int id, string title)
        {
            var detail = await _db.Tbl_CourseDetails.Where(a => a.Id == id && a.Title == title.Replace("§", " ").ToString() && a.ActivePassive == true).FirstOrDefaultAsync();
            if (detail == null)
            {
                return Redirect(Url.Content("~/" + "Error"));
            }
            courseVideos = await _db.Tbl_CourseVideos.Where(i => i.DetailId.Equals(detail.Id) && i.ActivePassive == true).ToListAsync();
        ...
        {
            var video = _db.Tbl_CourseVideos.Where(a => a.Id == id).FirstOrDefault();
            if (video.CountViews == null)
                video.CountViews = 1;
            else
                video.CountViews = video.CountViews + 1;
            _db.Tbl_CourseVideos.Update(video);
            _db.SaveChanges();
            //_CourseVideoJavaScript.cshtml
