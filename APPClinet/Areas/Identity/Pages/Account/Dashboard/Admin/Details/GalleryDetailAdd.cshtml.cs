...
{
    [RoleAttribute("Admin")]
    public class GalleryDetailAddModel : PageModel
    {
        ...
        public GalleryDetailAddModel(APPClinetDbContext db,
        ...
        {
            var subcategories = await _db.Tbl_GallerySubCategories.FirstOrDefaultAsync(c => c.Id.Equals(id));
        ...
        public async Task<IActionResult> OnPostAsync(int id, IFormFile ImageLink, IFormFile Image)
        {
            ...
            {
                GalleryDetails detail = new GalleryDetails();
                        ...
                        if (Image != null)
                        {
                            if (Image.ContentType == "image/jpeg")
                            {
                                if (Image.Length <= 1048576) // 1 MB -> 1048576 Bit
                                {
                                    ...
                                    int count = _db.Tbl_GalleryDetails.Count();
                                    ...
                                    _db.Tbl_GalleryDetails.Add(detail);
                ...
                // Send Emails
                string callbackUrl = Url.PageLink().Replace("/Identity/Account/Dashboard/Admin/Details/GalleryDetailAdd", "/Identity/Search/Details/GalleryDetailsInfoView?id=" + detail.Id + "&title=" + detail.Title.Replace(" ", "§").ToString()).ToString();
                string urlpagelink = Url.PageLink().Replace("/Identity/Account/Dashboard/Admin/Details/GalleryDetailAdd", "/").ToString();
                var users = await _db.Users.Where(u => u.EmailConfirmed == true && u.Newsletter == true).ToListAsync();
                _iuser.OnGetSendNewsletterGalleryEmailUsers(detail.Id, users, callbackUrl, urlpagelink);
                // Send Emails
