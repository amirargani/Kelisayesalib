...
namespace APPClinet.Areas.Identity.Pages.Account.Dashboard.Admin.Details
{
    ...
    public class TeamDetailEditModel : PageModel
    {
        ...
        {
            var detail = _db.Tbl_TeamDetails.Where(a => a.Id.Equals(id)).FirstOrDefault();
            if (detail == null)
            {
            ...
            if (Input.Text != null)
            {
                ...
                if (ImageProfile != null)
                {
                    if (ImageProfile.ContentType == "image/jpeg")
                    {
                        ...
                        {
                            var deleteimageprofile = Path.Combine(_environment.WebRootPath, "uploads\\images\\details\\teams\\" + detail.ImageProfile);
                            if (System.IO.File.Exists(deleteimageprofile))
                            {
                                System.IO.File.Delete(deleteimageprofile);
                            }
                            ...
                            if (Image != null)
                            {
                                if (Image.ContentType == "image/jpeg")
                                {
                                    ...
                                    {
                                        var deleteimage = Path.Combine(_environment.WebRootPath, "uploads\\images\\details\\teams\\" + detail.Image);
                ...
                return Redirect(Url.Content("~/") + "Identity/Account/Dashboard/Admin/Details/TeamDetailsView");
            }
            else
            {
                await OnGetAsync(id);
