...
{
    [RoleAttribute("Admin")]
    public class EventDetailAddModel : PageModel
    {
        ...
        public EventDetailAddModel(APPClinetDbContext db,
        ...
        public class InputModel
        {
            ...
            //countrycode.org
            //stackoverflow.com/questions/18584492/regular-expression-for-asp-net-for-international-local-phone-numbers
            // ^\s*\+?[0-9]\d?[- .]?(\([2-9]\d{2}\)|[2-9]\d{2})[- .]?\d{3}[- .]?\d{4}$
            //stackoverflow.com/questions/30532161/mvc-validation-make-regularexpression-numeric-only-on-string-field
            //debuggex.com/ -> ^\s*\+?[0-9]\d{1,2}?[- .]?\d{3,4}[- .]?\d{7,8}$ +49 163 2788229 || +49 176 11407721
            [RegularExpression(@"^\s*\+?[0-9]\d{1,2}?[- .]?\d{3,4}[- .]?\d{7,8}$", ErrorMessage = faMessage.RequiredMsgInfo)]
            [Display(Name = faMessage.PhoneNumber)]
            public string PhoneNumber { get; set; }
            [DataType(DataType.PhoneNumber)]
            // -> ^\s*\+?[0-9]\d{1,2}?[- .]?\d{3,4}[- .]?\d{7,8}$  +49 451 8104030
            [RegularExpression(@"^\s*\+?[0-9]\d{1,2}?[- .]?\d{3,4}[- .]?\d{7,8}$", ErrorMessage = faMessage.RequiredMsgInfo)]
        ...
        public async Task<IActionResult> OnPostAsync(int id, IFormFile Image)
        {
            if (Input.Text != null)
            {
                ...
                if (Image != null)
                {
                    if (Image.ContentType == "image/jpeg")
                    {
                        if (Image.Length <= 1048576) // 1 MB -> 1048576 Bit
                        {
                            ...
                            //var directoryuploads = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot\\uploads\\images\\details\\events");
                            string filename = "events-" + codeimage + "." + Image.FileName.Split('.').Last();
                            using (var fileStream = new FileStream(Path.Combine(uploads, filename), FileMode.Create))
                            {
                                Image.CopyTo(fileStream);
                            }
                            int count = _db.Tbl_EventDetails.Count();
                            ...
                            _db.Tbl_EventDetails.Add(detail);
                ...
                // Send Emails
                string callbackUrl = Url.PageLink().Replace("/Identity/Account/Dashboard/Admin/Details/EventDetailAdd", "/Identity/Search/Details/EventDetailsView?titlesubcategory=" + subcategories.TitleSubCategory.Replace(" ", "§").ToString()).ToString();
                string urlpagelink = Url.PageLink().Replace("/Identity/Account/Dashboard/Admin/Details/EventDetailAdd", "/").ToString();
