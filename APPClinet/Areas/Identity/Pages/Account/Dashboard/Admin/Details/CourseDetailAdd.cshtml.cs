...
{
    [RoleAttribute("Admin")]
    public class CourseDetailAddModel : PageModel
    {
        ...
        public CourseDetailAddModel(APPClinetDbContext db,
        ...
        {
            var subcategories = await _db.Tbl_CourseSubCategories.FirstOrDefaultAsync(c => c.Id.Equals(id));
            ...
            if (ImageLink != null && Input.Text != null)
            {
                ...
                if (ImageLink.ContentType == "image/jpeg")
                {
                    if (ImageLink.Length <= 1048576) // 1 MB -> 1048576 Bit
                    {
                        ...
                        string filenameimagelink = "courses-g-" + codeimageprofile + "." + ImageLink.FileName.Split('.').Last();
                        ...
                        if (Image != null)
                        {
                            if (Image.ContentType == "image/jpeg")
                            {
                                if (Image.Length <= 1048576) // 1 MB -> 1048576 Bit
                                {
                                    ...
                                    string filename = "courses-" + codeimage + "." + Image.FileName.Split('.').Last();
                                    using (var fileStream = new FileStream(Path.Combine(uploads, filename), FileMode.Create))
                                    {
                                        Image.CopyTo(fileStream);
                                    }
                                    int count = _db.Tbl_CourseDetails.Count();
                                    ...
                                    _db.Tbl_CourseDetails.Add(detail);
                ...
                // Send Emails
                string callbackUrl = Url.PageLink().Replace("/Identity/Account/Dashboard/Admin/Details/CourseDetailAdd", "/Identity/Search/Details/CourseDetailsInfoView?id=" + detail.Id + "&title=" + detail.Title.Replace(" ", "§").ToString()).ToString();
                string urlpagelink = Url.PageLink().Replace("/Identity/Account/Dashboard/Admin/Details/CourseDetailAdd", "/").ToString();
                var users = await _db.Users.Where(u => u.EmailConfirmed == true && u.Newsletter == true).ToListAsync();
                _iuser.OnGetSendNewsletterCourseEmailUsers(detail.Id, users, callbackUrl, urlpagelink);
                // Send Emails
                return Redirect(Url.Content("~/") + "Identity/Account/Dashboard/Admin/SubCategories/CourseSubCategoriesView");
            }
            else
            {
                await OnGetAsync(id);
