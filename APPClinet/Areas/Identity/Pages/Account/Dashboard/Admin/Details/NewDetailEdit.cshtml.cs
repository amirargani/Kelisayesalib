...
namespace APPClinet.Areas.Identity.Pages.Account.Dashboard.Admin.Details
{
    ...
    public class NewDetailEditModel : PageModel
    {
        ...
        {
            var detail = _db.Tbl_NewDetails.Where(a => a.Id.Equals(id)).FirstOrDefault();
            if (detail == null)
            {
            ...
            if (Input.Text != null)
            {
                ...
                if (ImageLink != null)
                {
                    if (ImageLink.ContentType == "image/jpeg")
                    {
                        ...
                        {
                            var deleteimagelink = Path.Combine(_environment.WebRootPath, "uploads\\images\\details\\news\\" + detail.ImageLink);
